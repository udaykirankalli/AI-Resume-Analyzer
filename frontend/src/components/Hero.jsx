    ...
    <section className="relative isolate min-h-screen overflow-hidden bg-[#080b1f] px-6 pb-20 pt-32 text-white">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-8rem)] max-w-6xl items-center justify-center">
        <div className="max-w-4xl rounded-[2rem] border border-white/15 bg-white/[0.07] px-7 py-12 text-center shadow-[0_30px_100px_rgba(3,6,23,0.5)] backdrop-blur-xl md:px-14 md:py-16">
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-300/10 px-4 py-2 text-sm font-medium text-cyan-100">
            <Sparkles className="h-4 w-4" />
            AI-powered resume intelligence
          </div>
          <h1 className="text-4xl font-semibold tracking-tight text-white sm:text-6xl md:text-7xl">
            Make every application
            <span className="mt-2 block bg-gradient-to-r from-cyan-200 via-sky-200 to-blue-200 bg-clip-text text-transparent">tell your strongest story.</span>
          </h1>
          ...
          <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-12 rounded-xl bg-cyan-300 px-6 font-semibold text-slate-950 hover:bg-cyan-200">
          ...
          <div className="mt-10 grid gap-3 text-left sm:grid-cols-3">
            {proofPoints.map((point) => <div key={point} className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950/25 px-4 py-3 text-sm text-slate-200"><CheckCircle2 className="h-4 w-4 shrink-0 text-cyan-200" />{point}</div>)}
          </div>
        </div>
      </div>
    </section>
